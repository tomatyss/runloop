...
- **Debian packages:** Build via `packaging/systemd/` using `just deb`. Verify
  install on Debian 13/trixie (x86_64 and arm64), ensure `runloopd.service`
  starts, and exercise an `apt upgrade` from the prior release to confirm a
  running daemon is restarted automatically.
