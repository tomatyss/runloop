...
1. Propose new schema via PR updating this file (and ideally
...
2. For third-party provisional IDs (`0x0400–0x0FFF`), include:
   ...
   - Sample payload
3. For vendor-defined IDs (`0x1000+`), Runloop runtime enforces the presence of
   `schema_hash` (e.g., SHA-256 of canonical schema) and `vendor` string inside
   the payload body.
