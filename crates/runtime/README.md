...
| `exec_spawn`           | Runs the provided command via `sh -c`, gated by `exec` caps; returns the exit code or a negative error when spawn fails or the process is killed. Stdout/stderr are emitted to host logs (truncated).                                                                                                   |
| `exec_spawn_capture`   | Same semantics as `exec_spawn` but copies stdout/stderr into guest buffers (little-endian `u32` length prefix + bytes), enabling agents to react to command output directly. Each buffer is limited to 64 KiB; exceeding the cap returns `EXEC_ENOSPACE`.                                            |
