...
- **RMP (Runloop Message Protocol) v1** with header table, framing example, and message registry.  
...
- **RMP v0 header (M0.2):** includes `created_at_ms`, fixed header version/size; TTL enforcement based on receiver clock with `created_at_ms`.  
...
- **Routing policy:** shell fast‑path togglable; “why this route?” always producible.
